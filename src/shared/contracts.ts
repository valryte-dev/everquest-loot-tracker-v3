...
export interface AppSnapshot { settings:Record<string,string>; members:Member[]; loot:Loot[]; splits:Split[]; tracked:TrackedLoot[]; history:History[]; items:MasterItem[]; inventory:InventoryItem[]; spells:Spell[]; wts:WtsGroup[]; aliases:Alias[]; mobs:string[]; logs:AppLog[]; imports:ImportRecord[]; merchant:MerchantMessage[]; compound:CompoundWorkspace }
