...
export interface DamageEncounter { id:number; character:string; mobName:string; startedAt:string; endedAt?:string; lastDamageAt:string; totalDamage:number; meleeDamage:number; spellDamage:number; hitCount:number; maxHit:number; incomingDamage?:number; incomingHitCount?:number; incomingMaxHit?:number; procCount?:number; directProcDamage?:number; dotDamage?:number; procDotDamage?:number; totalProcDamage?:number; spellMetrics?:DamageSpellMetric[]; trackedSpells?:TrackedSpellActivity[]; outcome:"active"|"slain"|"playerDeath"|"disengaged"; sourceFile:string; weapons:string[]; players:DamageParticipant[]; damageTargets?:DamageTarget[]; activeDots?:ActiveDot[] }
