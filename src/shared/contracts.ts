...
export interface ActiveDot { id:number; spellName:string; targetName:string; casterName:string; attributionMethod:"item_glow"|"direct_cast"|"next_attack"|"unknown"; landedAt:string; expiresAt:string; damagePerTick:number; tickIntervalSeconds:number; totalTicks:number; ticksApplied:number; inferenceEnabled:boolean; status:string }
export interface DamageEncounter { id:number; character:string; mobName:string; startedAt:string; endedAt?:string; lastDamageAt:string; totalDamage:number; meleeDamage:number; spellDamage:number; hitCount:number; maxHit:number; incomingDamage?:number; incomingHitCount?:number; incomingMaxHit?:number; outcome:"active"|"slain"|"playerDeath"|"disengaged"; sourceFile:string; weapons:string[]; players:DamageParticipant[]; damageTargets?:DamageTarget[]; activeDots?:ActiveDot[] }
...
export interface InventoryItem { character:string; importedAt:string; id:number; location:string; itemName:string; itemId?:number; count:number; slots?:number; valuePp?:number; valueBasis?:string; valueSamples?:number }
...
export interface DatabaseCleanupPreview { cutoff:string; outgoingRows:number; incomingRows:number; totalRows:number; estimatedPayloadBytes:number; oldestAt?:string; newestAt?:string; encounterSummariesPreserved:boolean }export interface DotTrainingLine { lineNumber:number; sourceOffset:number; happenedAt?:string; rawLine:string; status:"dot"|"recognized"|"ignored"|"invalid"; parserEvent:string; summary:string; decisions:string[] }
export interface DotTrainingApplication { id:number; encounterId:number; spellName:string; targetName:string; casterName:string; attributionMethod:"item_glow"|"direct_cast"|"next_attack"|"unknown"; landedAt:string; expiresAt:string; damagePerTick:number; tickIntervalSeconds:number; totalTicks:number; ticksApplied:number; inferenceEnabled:boolean; status:string; sourceOffset:number }
export interface DotTrainingEvent { id:number; encounterId:number; happenedAt:string; attacker:string; damageType:string; attack:string; damage:number; inferred:boolean; tickIndex?:number; sourceOffset:number }
export interface DotTrainingParticipant { name:string; totalDamage:number; hitCount:number }
export interface DotTrainingEncounter { id:number; mobName:string; startedAt:string; endedAt?:string; lastDamageAt:string; totalDamage:number; meleeDamage:number; spellDamage:number; hitCount:number; outcome:string; participants:DotTrainingParticipant[]; events:DotTrainingEvent[]; dots:DotTrainingApplication[] }
export interface DotTrainingReport { activeCharacter:string; lineCount:number; recognizedCount:number; ignoredCount:number; dotProfileCount:number; projectedAllTicks:boolean; projectedThrough?:string; lines:DotTrainingLine[]; encounters:DotTrainingEncounter[]; warnings:string[] }
