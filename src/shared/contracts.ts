...
export interface DatabaseCleanupPreview { cutoff:string; outgoingRows:number; incomingRows:number; totalRows:number; estimatedPayloadBytes:number; oldestAt?:string; newestAt?:string; encounterSummariesPreserved:boolean }export type LoadingState<T>={kind:"loading"}|{kind:"ready";value:T}|{kind:"error";message:string};
