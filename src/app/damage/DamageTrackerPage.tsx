...
import type {AppSnapshot,DamageEncounter,DamageEncounterDetail,DamageEvent,DamageParticipant} from "../../shared/contracts";
...
function LiveFight({row,now,seenAt,view,newest,pinned,pin,unpin,close,open}:{row:DamageEncounter;now:number;seenAt:number;view:"compact"|"detail";newest:boolean;pinned:boolean;pin:()=>void;unpin:()=>void;close:()=>void;open:()=>void}){
 ...
 return <article className={"damage-live "+(!pinned&&idle>=30?"is-fading ":"")+(pinned?"is-pinned":"")}>
  ...
  <div className={"damage-live-metrics "+view}><div className="primary"><span>Group DPS</span><strong>{liveDps.toFixed(1)}</strong><small>{elapsed(duration)} elapsed</small></div><div><span>Group damage</span><strong>{num(row.totalDamage)}</strong><small>{row.hitCount.toLocaleString()} events</small></div><div className="mine"><span>My DPS</span><strong>{myDps.toFixed(1)}</strong><small>{row.character}</small></div><div className="mine"><span>My damage</span><strong>{num(myDamage)}</strong><small>{mine?.hitCount||0} events - {Math.round(myShare)}% share</small></div>{view==="detail"&&<><div><span>Group hits</span><strong>{row.hitCount.toLocaleString()}</strong></div><div><span>Group max hit</span><strong>{num(row.maxHit)}</strong></div></>}</div>
  ...
  {view==="detail"&&<><LiveTelemetry row={row} duration={duration}/><PlayerCharts players={row.players}/></>}
 </article>
}
