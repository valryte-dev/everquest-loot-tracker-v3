...
export function DamageTrackerPage({data,run}:{data:AppSnapshot;run:(a:string,p?:Record<string,unknown>)=>Promise<unknown>}){
 const[detail,setDetail]=useState<DamageEncounterDetail|null>(null),[loading,setLoading]=useState<number|null>(null),[error,setError]=useState(""),[scanning,setScanning]=useState(false),[scanProgress,setScanProgress]=useState<DamageScanProgress|null>(null);
 ...
 return <>
  ...
  <CollapsiblePanel id="current-player" label="Current player and equipped weapons"><CurrentPlayer character={data.settings.active_character} loadout={data.currentWeaponLoadout}/></CollapsiblePanel>
  <LiveFightArea rows={rows} activeCharacter={data.settings.active_character} view={liveView} meterDocked={meterPlacement==="docked"} open={open}/>
  <CollapsiblePanel id="lifetime-summary" label="Lifetime combat summary"><section className="damage-stats damage-stats-six">
   <article><span>Encounters</span><strong>{data.damageEncounterCount.toLocaleString()}</strong><small>{data.damageDistinctMobCount.toLocaleString()} distinct mobs{data.damageEncounterCount>rows.length?" - newest "+rows.length.toLocaleString()+" loaded":""}</small></article>
  ...
  <CollapsiblePanel id="cleric-chain" label="Complete Heal chain tracking"><ClericChainPanel calls={data.clericHealCalls||[]} encounters={rows} activeCharacter={data.settings.active_character} clearedAt={data.settings.ch_chain_cleared_at} run={run}/></CollapsiblePanel>
  <CollapsiblePanel id="analytics" label="Historical damage analytics"><section className="damage-overview"><article className="card"><header><div><h2>Top mobs by damage</h2><p>Combined damage across all encounters.</p></div></header>{ranked.length?<div className="damage-ranking">{ranked.map((r,i)=><div key={r.mob}><b>{i+1}</b><span title={r.mob}>{r.mob}<small>{r.count} encounter{r.count===1?"":"s"}</small></span><i><em style={{width:`${r.damage/max*100}%`}}/></i><strong>{num(r.damage)}</strong></div>)}</div>:<div className="damage-empty">No combat damage discovered yet.</div>}</article><article className="card"><header><div><h2>Damage composition</h2><p>Melee compared with spell and non-melee damage.</p></div></header><Composition rows={rows}/></article></section></CollapsiblePanel>
  {error&&<div className="alert"><span>{error}</span><button onClick={()=>setError("")}>x</button></div>}
  <CollapsiblePanel id="recorded-encounters" label="Recorded encounters"><section className="card"><header><div><h2>Recorded encounters</h2><p>The newest 5,000 encounters are loaded for filtering and sorting; the lifetime total above is uncapped.</p></div><small>{data.settings.damage_tracker_last_scan_at?`Last scan ${when(data.settings.damage_tracker_last_scan_at)}`:"Initial scan pending"}</small></header><DataTable rows={rows} columns={columns} rowKey={r=>r.id} empty={scanning?"Scanning all logs...":"No damage encounters found yet."} actions={r=><IconButton icon="external" label={`View ${r.mobName} encounter`} disabled={loading===r.id} onClick={()=>open(r)}/>} /></section></CollapsiblePanel>
  {detail&&<Details row={detail} close={()=>setDetail(null)}/>}
 </>;
}

function CollapsiblePanel({id,label,children}:{id:string;label:string;children:ReactNode}){
 ...
 return <section className={"damage-collapsible"+(open?" open":"")}><button className="damage-collapse-toggle" aria-expanded={open} onClick={toggle}><span>{label}</span><i/></button>{open&&<div className="damage-collapse-content">{children}</div>}</section>;
}
...
function LiveFightArea({rows,activeCharacter,view,meterDocked,open}:{rows:DamageEncounter[];activeCharacter?:string;view:"compact"|"detail";meterDocked:boolean;open:(row:DamageEncounter)=>void}){
 ...
 const live=useMemo(()=>sortLiveEncountersByPlayerTarget(selectLiveEncounters(rows,activeCharacter,new Map([...observed.current].map(([id,value])=>[id,value.seenAt])),now,closed),activeCharacter),[rows,activeCharacter,now,closed]);
 const cards=useMemo(()=>{const found:DamageEncounter[]=[...live];pinned.forEach(id=>{const row=rows.find(r=>r.id===id);if(row&&!closed.includes(id)&&!found.some(value=>value.id===id))found.push(row)});return sortLiveEncountersByPlayerTarget(found,activeCharacter)},[live,pinned,rows,closed,activeCharacter]);
 return cards.length?<><section className="damage-engaged-strip" aria-label="Currently engaged mobs"><span>{live.length} engaged</span>{live.map((row,index)=><div key={row.id} className={index===0&&Boolean(own(row))?"target":""}><small>{index===0&&own(row)?"Target":"Engaged"}</small><strong title={row.mobName}>{row.mobName}</strong><b>{num(ownDamage(row))} mine</b></div>)}</section><section className="damage-live-stack" aria-label="Live fights">{cards.map(row=><LiveFight key={row.id} row={row} now={now} seenAt={observed.current.get(row.id)?.seenAt||now} view={view} meterDocked={meterDocked} newest={live.some(value=>value.id===row.id)} pinned={pinned.includes(row.id)} pin={()=>setPinned(ids=>ids.includes(row.id)?ids:[...ids,row.id])} unpin={()=>setPinned(ids=>ids.filter(id=>id!==row.id))} close={()=>{setPinned(ids=>ids.filter(id=>id!==row.id));setClosed(ids=>ids.includes(row.id)?ids:[...ids,row.id])}} open={()=>open(row)}/>)}</section></>:null;
}

function LiveFight({row,now,seenAt,view,meterDocked,newest,pinned,pin,unpin,close,open}:{row:DamageEncounter;now:number;seenAt:number;view:"compact"|"detail";meterDocked:boolean;newest:boolean;pinned:boolean;pin:()=>void;unpin:()=>void;close:()=>void;open:()=>void}){
 ...
 return <article className={"damage-live "+(!pinned&&idle>=30?"is-fading ":"")+(pinned?"is-pinned":"")}>
  ...
  {!collapsed&&<div className="damage-live-body"><div className={"damage-live-metrics "+view}><div className="mine primary"><span>My DPS</span><strong>{myDps.toFixed(1)}</strong><small>{row.character} - {elapsed(duration)} elapsed</small></div><div className="mine"><span>My damage</span><strong>{num(myDamage)}</strong><small>{mine?.hitCount||0} events - {Math.round(myShare)}% share</small></div><div><span>Group DPS</span><strong>{liveDps.toFixed(1)}</strong><small>{elapsed(duration)} elapsed</small></div><div><span>Group damage</span><strong>{num(row.totalDamage)}</strong><small>{row.hitCount.toLocaleString()} events</small></div>{view==="detail"&&<><div><span>Group hits</span><strong>{row.hitCount.toLocaleString()}</strong></div><div><span>Group max hit</span><strong>{num(row.maxHit)}</strong></div></>}</div>
  ...
  {view==="detail"&&<><LiveTelemetry row={row} duration={duration}/><PlayerCharts players={row.players}/><IncomingDamageChart row={row}/></>}</div>}
 </article>
}
